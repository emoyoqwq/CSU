...
import * as matchSchema from '../../schemas/MatchResponse';
import * as ongoingMatchesSchema from '../../schemas/OngoingMatchesResponse';
import * as matchHistorySchema from '../../schemas/MatchHistoryResponse';
import { LOG } from './constants';

const SCHEMAS = {
    config: configSchema,
    match: matchSchema,
    ongoingMatches: ongoingMatchesSchema,
    matchHistory: matchHistorySchema
}

export async function sendAPIRequest(requestBody, serverUrl, type="POST") {
    const response = await sendRequest(requestBody, serverUrl, type);
...
export async function sendRequest(requestBody, serverUrl, type) {
    const fetchOptions = {
        method: type,
